...
import { CheckCircle2, XCircle, Target, TrendingUp, Clock, Home, RotateCcw, FileText, AlertCircle } from "lucide-react"
import { PageLoading } from "@/components/ui/page-loading"
import { EmptyState } from "@/components/ui/empty-state"
import { aiApi } from "@/lib/api/ai"
import type { WritingSubmissionResponse } from "@/types/ai"
...
function WritingSubmissionDetailContent() {
  ...
  const [data, setData] = useState<WritingSubmissionResponse | null>(null)
  ...
  useEffect(() => {
    const fetchSubmission = async () => {
      try {
        ...
        console.error("[Writing Submission] Failed to load:", error)
      } finally {
        setLoading(false)
      }
    }
    ...
    <AppLayout>
      <PageContainer maxWidth="4xl">
            ...
            {evaluation ? (
              <>
                ...
                <p className="text-muted-foreground">
                  {t("your_writing_has_been_evaluated") || "Your writing has been evaluated"}
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-muted-foreground">
                  {submission.status === "processing"
                    ? (t("please_wait_for_evaluation") || "Please wait while we evaluate your writing...")
                    : (t("evaluation_not_available") || "Evaluation is not available at this time")}
                </p>
              </>
            )}
          </CardHeader>
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center p-4 bg-blue-50 dark:bg-blue-950 rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">{t("task_achievement") || "Task Achievement"}</p>
                    <p className="text-2xl font-bold text-blue-600">{evaluation.task_achievement.toFixed(1)}</p>
                  </div>
                  <div className="text-center p-4 bg-green-50 dark:bg-green-950 rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">{t("coherence_cohesion") || "Coherence & Cohesion"}</p>
                    <p className="text-2xl font-bold text-green-600">{evaluation.coherence_cohesion.toFixed(1)}</p>
                  </div>
                  <div className="text-center p-4 bg-purple-50 dark:bg-purple-950 rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">{t("lexical_resource") || "Lexical Resource"}</p>
                    <p className="text-2xl font-bold text-purple-600">{evaluation.lexical_resource.toFixed(1)}</p>
                  </div>
                  <div className="text-center p-4 bg-orange-50 dark:bg-orange-950 rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">{t("grammatical_range") || "Grammatical Range"}</p>
                    <p className="text-2xl font-bold text-orange-600">{evaluation.grammatical_range.toFixed(1)}</p>
                  </div>
                </div>
              </>
            )}
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="font-semibold">{submission.task_type === "task1" ? "Task 1" : "Task 2"}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">{t("word_count") || "Word Count"}</p>
                <p className="font-semibold">{submission.word_count}</p>
              </div>
              <div>
        ...
        {evaluation && (
          <>
            ...
            <Card className="mb-8">
              ...
              <CardContent>
                <div className="prose prose-sm max-w-none">
                  <p className="whitespace-pre-wrap">{evaluation.detailed_feedback}</p>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button variant="outline" onClick={() => router.push("/ai/writing")}>
            <Home className="w-4 h-4 mr-2" />
            {t("back_to_prompts") || "Back to Prompts"}
          </Button>
          {evaluation && (
            <Button onClick={() => router.push(`/ai/writing/${submission.task_prompt_id || ""}`)}>
